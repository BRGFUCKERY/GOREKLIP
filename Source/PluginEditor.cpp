                                                ...
                                                juce::ComboBox& box)
{
    ...
    // Shared geometry for both sides so they are perfectly aligned
    const float iconSize   = bounds.getHeight() * 0.55f;
    const float iconRadius = iconSize * 0.5f;

    // Vertical center is identical for pentagram and "x1"
    const float centerY    = bounds.getCentreY();

    // Same inset from the right edge of each combo box
    ...
    if (name == "lookBox")
    {
        ...
        // Text is hidden (transparent colour) for lookBox;
        // only the pentagram is visible.
    }
    // ===================================
    ...
    {
        // Draw "x1/x2/..." manually so it lines up perfectly
        // with the pentagram and shares the same burn colour.
        ...
        // Slightly larger, bold text so it has similar visual weight
    ...
    else
    {
        ...
        // keep transparent background, let JUCE draw its normal text.
    }
}
    ...
    : AudioProcessorEditor (&p), processor (p)
{
    ...
    // so hide JUCE's default box text.
